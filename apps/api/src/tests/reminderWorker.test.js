...
beforeEach(() => {
  ...
  Object.assign(mockServiceInstance, {
    ...
    sendPendingFeedbackRequests: jest.fn().mockResolvedValue(),
...
describe('startReminderWorker', () => {
  it('immediately calls all four send methods at startup', async () => {
    interval = startReminderWorker();
    await flush();
    expect(mockServiceInstance.sendPendingReminders).toHaveBeenCalledTimes(1);
    expect(mockServiceInstance.sendPendingFeedbackRequests).toHaveBeenCalledTimes(1);
  ...
  it('continues processing when one method throws', async () => {
    ...
    expect(mockServiceInstance.sendPendingFeedbackRequests).toHaveBeenCalled();
  ...
  it('logs separate errors for each failing method', async () => {
    mockServiceInstance.sendPendingFeedbackRequests.mockRejectedValueOnce(new Error('feedback fail'));
    interval = startReminderWorker();
    await flush();
    expect(logger.error).toHaveBeenCalledWith('feedback_worker_error', expect.objectContaining({ message: 'feedback fail' }));
  });
});
